...
// NOTE: This test requires a live Supabase connection with real test accounts.
// Run manually when needed: npx vitest run src/test/integration/RealSupabase.test.ts
describe.skip('Real Supabase RLS & Isolation', () => {
  ...
  it('enforces RLS: User B cannot UPDATE User A data', async () => {
    ...
    await supabase.auth.signInWithPassword({ email: userB.email, password: userB.password });
    await supabase.from('customers').update({ name: 'Hacked' }).eq('id', customerA.id);
