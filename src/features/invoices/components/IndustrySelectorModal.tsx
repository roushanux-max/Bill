...
export default function IndustrySelectorModal({ onClose, canClose = true }: IndustrySelectorModalProps) {
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                    ...
                    {canClose && (
                        <button onClick={onClose} className="text-slate-400 hover:text-slate-700 bg-slate-50 p-2 rounded-full">
                            <X size={20}/>
                        </button>
                    )}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                    {domains.map(d => (
                        <button 
                            ...
                            style={activeDomain === d.id ? { borderColor: settings.primaryColor, backgroundColor: `${settings.primaryColor}10` } : {}}
                        >
                ...
                {canClose && (
                    <button onClick={onClose} className="w-full py-3 text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl font-bold transition-colors">
                        Close
                    </button>
                )}
            </div>
