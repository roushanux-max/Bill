...
export function generateInvoicePDF(
  ...
  // Helper to draw icon + text in a row
  const drawFootRow = (icon: string, text: string) => {
    setFont('bold', 12, primary);
    pdf.text(icon, margin, fy);
    setFont('bold', 10, textDark);
    pdf.text(text, margin + 8, fy);
    fy += 7;
  };

  if (storeInfo.phone) drawFootRow('📞', storeInfo.phone);
  if (storeInfo.email) drawFootRow('✉', storeInfo.email);
  if (storeInfo.address) drawFootRow('📍', storeInfo.address);

  setFont('bold', 7, textLight);
  pdf.text(`Generated by Invoice`, 210 - margin, footerY + footerH - 5, { align: 'right' });
