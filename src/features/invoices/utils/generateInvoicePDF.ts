...
export function generateInvoicePDF(
  ...
  pdf.text(formatDateForDisplay(invoice.date), 210 - margin, logoY + 20, { align: 'right' });
  ...
  // Status Info (Aligned right)
  let metaY = headerH + 15 + 8;
  setFont('bold', 10, textMid);
  pdf.text('STATUS:', 210 - margin, metaY, { align: 'right' });
  setFont('bold', 11, textDark);
  pdf.text((invoice.status || 'UNPAID').toUpperCase(), 210 - margin, metaY + 5, { align: 'right' });

  if (activeDomain !== 'furniture' && invoice.dueDate) {
    metaY += 15;
    setFont('bold', 10, textMid);
  ...
  const items = [];
  if (storeInfo.phone) items.push({ icon: '📞', text: storeInfo.phone });
  if (storeInfo.email) items.push({ icon: '✉', text: storeInfo.email });
  if (storeInfo.address) items.push({ icon: '📍', text: storeInfo.address });
  ...
  items.forEach((item) => {
    ...
    setFont('bold', 8, textDark); // Slightly smaller text for row layout
    pdf.text(item.text, fx + iconSpace, fy);
    fx += pdf.getTextWidth(item.text) + iconSpace + footerGap;
  });

  setFont('bold', 7, textLight);
  pdf.text(`Invoice No: #${invoice.invoiceNumber || 'INV-0001'}`, margin, footerY + footerH - 5);
  pdf.text(`Generated by Invoice • Page 1 of 1`, 210 - margin, footerY + footerH - 5, { align: 'right' });
