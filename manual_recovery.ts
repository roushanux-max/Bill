...
async function runRecovery() {
  ...
  if (storeError || !stores || stores.length === 0) {
    console.error("No stores found to map data to. Please ensure you are logged in or provide a store ID.");
    return;
  }
  ...
  if (zeroTotalInvoices && zeroTotalInvoices.length > 0) {
     for (const inv of zeroTotalInvoices) {
       ...
       const { data: customer } = await supabase.from('customers').select('name').eq('id', inv.customer_id).maybeSingle();
